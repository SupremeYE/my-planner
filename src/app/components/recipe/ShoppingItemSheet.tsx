...
interface ShoppingItemSheetProps {
  item: ShoppingItem | null;       // null = 신규 추가(직접 추가)
  onSave: (item: ShoppingItem) => void;
  onDelete?: (id: string) => void;
  onClose: () => void;
}

export function ShoppingItemSheet({ item, onSave, onDelete, onClose }: ShoppingItemSheetProps) {
  const { t } = useTheme();
  const isEdit = !!item;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      sourceLabel: item?.sourceLabel ?? '직접 추가',
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-end p-0 lg:items-center lg:p-4"
      ...
      <style>{`@keyframes shopItemSheetUp{from{transform:translateY(100%)}to{transform:translateY(0)}}
        @media (max-width:1023px){.shop-item-sheet{animation:shopItemSheetUp .26s ease-out}}`}</style>
      <div className="shop-item-sheet shadow-2xl overflow-y-auto w-full max-w-full rounded-t-2xl
        ...
        <div className="flex items-center justify-between gap-2 px-4 lg:px-5 pb-3 sticky top-0 z-10"
          ...
          <h2 className="flex-1 text-center lg:flex-none lg:text-left" style={{ fontSize: 17, fontWeight: 700, color: t.text }}>
            {isEdit ? '장보기 항목 수정' : '장보기 항목 추가'}
          </h2>
          <button type="submit" form="shop-item-form" disabled={submitting}
        ...
        <form id="shop-item-form" onSubmit={handleSubmit}
          ...
          <div className="flex gap-3">
            ...
            <div className="w-28">
              ...
              <input value={unit} onChange={e => setUnit(e.target.value)} placeholder="개·g" style={fieldStyle} />
            </div>
          </div>
