  ...
  return (
    <div className="min-h-full" style={{ backgroundColor: t.bg }}>
      ...
      <div className="px-4 pt-5 pb-24">
        ...
        {b.loading ? (
          <div className="grid grid-cols-2 gap-2.5">{Array.from({ length: 2 }).map((_, i) => <div key={i} className="rounded-2xl" style={{ height: 132, backgroundColor: t.lavenderTint }} />)}</div>
        ) : (
          <SpecialCareSection
        ...
        {b.loading ? (
          <div className="flex gap-2.5">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="rounded-2xl flex-shrink-0" style={{ width: 118, height: 168, backgroundColor: t.lavenderTint }} />)}</div>
        ) : (
          <BeautyShelfPreview
