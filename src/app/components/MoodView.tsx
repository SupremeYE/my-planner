...
function VoiceInputButton({ onResult, disabled }: { onResult: (text: string) => void; disabled?: boolean }) {
  ...
  return (
    <button type="button" onClick={toggle} disabled={disabled || isBusy}
      ...
      style={{ width: 30, height: 30, backgroundColor: isRec ? '#fee2e2' : t.surfaceMuted, border: `1px solid ${isRec ? '#fca5a5' : t.borderLight}`, color: isRec ? '#ef4444' : t.textMuted }}>
      {isRec
                ...
                {[1, 2, 3, 4, 5].map(level => (
                  <button key={level} onClick={() => setEnergyLevel(level)}
                    ...
                    <div className="rounded-full transition-all"
                      style={{ width: energyLevel === level ? 40 : 32, height: energyLevel === level ? 40 : 32, backgroundColor: energyLevel === level ? t.accent : t.surfaceMuted, border: `2px solid ${energyLevel === level ? t.accent : t.borderLight}` }} />
        ...
        <div className="flex gap-3 px-5 lg:px-8 pt-3 pb-8 lg:pb-6" style={{ borderTop: `1px solid ${t.borderLight}` }}>
          {step > 1 ? (
            <button onClick={() => setStep(s => s - 1)}
              ...
              style={{ backgroundColor: t.surfaceMuted, color: t.textSub, fontSize: 14, fontWeight: 600, border: `1px solid ${t.borderLight}` }}>
  ...
  const CalendarGrid = (
    <div>
      ...
      <div className="grid grid-cols-7 gap-px rounded-xl overflow-hidden" style={{ backgroundColor: t.borderLight }}>
        {cells.map((day, i) => {
          if (!day) return <div key={i} style={{ backgroundColor: t.surfaceMuted, minHeight: 44 }} />;
          ...
          return (
            <div key={i} onClick={() => setSelectedDate(isSelected ? null : ds)}
              ...
              style={{ minHeight: 44, backgroundColor: color ? color.bg : t.surfaceMuted, outline: isSelected ? `2px solid ${t.accent}` : isToday ? `2px solid ${t.accent}88` : 'none', outlineOffset: -1, padding: '3px 4px' }}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          {(['this-month', 'last-month', '14-days', 'custom'] as PeriodFilter[]).map(key => (
            <button key={key} onClick={() => setPeriodFilter(key)}
              ...
              style={{ fontSize: 12, fontWeight: periodFilter === key ? 700 : 400, backgroundColor: periodFilter === key ? t.accent : t.surfaceMuted, color: periodFilter === key ? '#fff' : t.textSub, border: `1px solid ${periodFilter === key ? t.accent : t.borderLight}` }}>
              {periodLabels[key]}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={i} style={cardBase} className="flex flex-col items-center justify-center py-4 text-center">
                <span style={{ fontSize: 20, fontWeight: 700, color: t.text, fontFamily: t.fontStat, display: 'block' }}>{s.value}</span>
                ...
                {topTags.map(([tag, count], i) => {
                  ...
                  return (
                    <div key={tag} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full"
                      style={{ backgroundColor: color?.bg ?? t.surfaceMuted, border: `1px solid ${color?.accent ? color.accent + '55' : t.borderLight}` }}>
...
export function MoodView() {
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-20" style={{ fontFamily: t.fontBody }}>
      {/* Header */}
      ...
      <div className="flex gap-1.5 px-5 mb-4">
        {tabs.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            ...
            style={{ fontSize: 13, fontWeight: tab === key ? 600 : 400, backgroundColor: tab === key ? t.accent : t.surfaceMuted, color: tab === key ? '#fff' : t.textSub }}>
            {label}
          </button>
        ))}
      </div>
