...
function AssignDayPopover({
  ...
  return (
    <div
      ref={ref}
      className="absolute right-0 top-full mt-1 z-30 rounded-xl p-2 shadow-lg"
...
// ── Unassigned Todo Item (draggable + assign button) ──
function UnassignedTodoItem({
  todo, weekDays, onAssign, isDragging, tags, projects,
}: {
  todo: Todo;
  weekDays: Date[];
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex-shrink-0">
          ...
          {showPopover && (
            <AssignDayPopover
              weekDays={weekDays}
              onAssign={(date) => { onAssign(todo.id, date); setShowPopover(false); }}
...
// ── Draggable Todo Card (improved: removed transition-opacity, added willChange) ──
function DraggableTodoCard({
  todo, tags, projects, isDragging, onClick,
}: {
  todo: Todo;
  tags: TagType[];
  projects: { id: string; name: string; color: string }[];
...
// ── Day Column (droppable, improved animation) ──
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isDraggingOverMe
          ...
          : undefined,
        // 드래그 중에는 box-shadow 애니메이션 끄고, border/bg만 빠르게
...
// ── Weekly Goals Section (exported for MonthlyView reuse) ──
...
export function WeeklyView() {
  const {
    todos, setSelectedDate, tags, projects, updateTodo,
  } = usePlanner();
  const { t } = useTheme();
  ...
  // 미지정 할일: date 없고 활성 상태인 것
  const unassignedTodos = todos.filter(
    td => !td.date && td.status !== 'cancelled' && td.status !== 'done' && td.status !== 'backlog',
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ backgroundColor: t.bg }}>
      ...
      {/* Body: DndContext가 왼쪽 패널 + 오른쪽 칸반 전체를 감쌈 */}
      <DndContext
        sensors={sensors}
      ...
      >
        <div className="flex flex-1 overflow-hidden">
          {/* Left: 미지정 할일 패널 */}
          <div
            className="flex flex-col overflow-y-auto p-4 flex-shrink-0"
              ...
              >
                ...
                {unassignedTodos.length > 0 && (
                  <span
                    ...
                    style={{ fontSize: 10, backgroundColor: t.accent, color: '#fff', fontWeight: 700 }}
                  >
                  ...
                  <>
                    {unassignedTodos.map(todo => (
                      <UnassignedTodoItem
                        ...
                        todo={todo}
                        weekDays={days}
                        onAssign={(id, date) => updateTodo(id, { date })}
                        isDragging={todo.id === activeDragId}
                        tags={tags}
                        projects={projects}
                      />
                    ))}
                    <p style={{ fontSize: 10, color: t.textMuted, lineHeight: 1.5, paddingTop: 2 }}>
                      드래그하거나 배정 버튼으로 날짜를 지정하세요.
                    </p>
                  </>
                )}
              </div>
