...
import { ChevronLeft, ChevronRight, Plus, Trash2, ChevronDown, ChevronUp, BarChart2 } from 'lucide-react';
import { usePlanner } from '../store';
import { useTheme } from '../ThemeContext';

export function MonthlyView() {
  const { monthlyGoals, weeklyGoals, habits, todos, addMonthlyGoal, deleteMonthlyGoal, toggleWeeklyGoal, deleteWeeklyGoal } = usePlanner();
  const { t } = useTheme();
  ...
  const monthTodos = todos.filter(t => t.date && t.date.startsWith(currentMonth));
  const doneTodos = monthTodos.filter(t => t.status === 'done');
  const totalTodos = monthTodos.filter(t => t.status !== 'backlog' && t.status !== 'cancelled');
  ...
  const getHabitMonthStats = (habit: { checkedDates: string[] }) => {
    const checkedThisMonth = habit.checkedDates.filter(d => d.startsWith(currentMonth)).length;
    return { count: checkedThisMonth, pct: Math.round((checkedThisMonth / daysInMonth) * 100) };
  };

  return (
    <div style={{ minHeight: '100%', backgroundColor: t.bg }}>
      {/* Header */}
      <div className="sticky top-0 z-10 px-4 py-4 flex items-center justify-between" style={{ backgroundColor: t.sidebar, borderBottom: `1px solid ${t.border}` }}>
        <button onClick={() => setViewDate(subMonths(viewDate, 1))} className="p-2 rounded-xl hover:bg-[#F0EBE3]">
          <ChevronLeft size={18} color="#888" />
        </button>
        <div style={{ fontSize: 17, fontWeight: 700, color: '#2D2D2D' }}>
          {format(viewDate, 'yyyy년 M월')}
        </div>
        <button onClick={() => setViewDate(addMonths(viewDate, 1))} className="p-2 rounded-xl hover:bg-[#F0EBE3]">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm" style={{ border: '1px solid #F0EBE3' }}>
                    ...
                    <div className="p-3 space-y-2" style={{ backgroundColor: '#F9F6F1', borderTop: '1px solid #F0EBE3' }}>
                      {subGoals.map(sg => (
                        <div key={sg.id} className="flex items-center gap-2 p-2.5 rounded-xl bg-white">
                          ...
                          <span style={{ flex: 1, fontSize: 13, color: sg.done ? '#888' : '#2D2D2D', textDecoration: sg.done ? 'line-through' : 'none' }}>
                            {sg.text}
                          </span>
          ...
          <div className="flex gap-2 mt-4">
            ...
            />
            <button onClick={handleAddGoal} className="px-3 py-2 rounded-xl flex items-center gap-1"
                      ...
                      {Array.from({ length: daysInMonth }, (_, i) => {
                        ...
                        return (
                          <div key={day} className="flex flex-col items-center">
                            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: checked ? '#C8A97E' : '#F0EBE3' }} title={dateStr} />
                          </div>
                        );
                      })}
                    </div>
