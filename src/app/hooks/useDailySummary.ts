...
// 8개 도메인의 summaryForDate 를 선택된 "논리 날짜" 기준으로 Promise.all 병렬 조회한다.
...
const EMPTY: DailySummaryState = {
  ...
  reading: null, culture: null, music: null, places: null,
};
...
export function useDailySummary(date: string) {
  ...
  useEffect(() => {
    ...
    ]).then(([condition, food, sleep, workout, reading, culture, music, places]) => {
      if (cancelled) return; // 날짜가 바뀌면 이전 응답 무시(스테일 방지)
      setState({ loading: false, condition, food, sleep, workout, reading, culture, music, places });
    });
