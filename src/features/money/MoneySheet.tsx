...
// padClass/maxVh: 모바일을 기존 시트와 정확히 일치시키기 위한 패딩·최대높이 오버라이드(기본 20/20/28·90vh).
export function MoneySheet({ onClose, size = 'md', padClass = 'pt-5 px-5 pb-7 lg:p-7', maxVh = 90, children }: {
  onClose: () => void; size?: MoneySheetSize; padClass?: string; maxVh?: number; children: React.ReactNode;
}) {
  const { t } = useTheme();
