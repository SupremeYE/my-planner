...
//   expense_categories?: string[],   // 선택 가능한 지출 카테고리 이름 목록
//   income_categories?: string[],    // 선택 가능한 수입 카테고리 이름 목록
// }
// 출력(항상 200 JSON, 실패도 graceful):
//   성공 → { ok:true, tx:{ type, amount, category, memo, paymentMethod, spentAt, emoji } }
...
function promptFor(
  ...
  const catLine = [
    `- 지출 카테고리 후보: ${expenseCats.length ? expenseCats.join(', ') : '(없음)'}`,
    `- 수입 카테고리 후보: ${incomeCats.length ? incomeCats.join(', ') : '(없음)'}`,
    '- category 는 반드시 위 후보 중 가장 알맞은 "이름 하나"를 그대로 골라. 애매하면 null.',
  ].join('\n');
  ...
  const common = [
    ...
    '  - category: 위 규칙대로 후보 중 하나 또는 null.',
    ...
    '출력은 JSON 객체 하나만: {"type":"expense","amount":50000,"category":"식비","memo":"갈비 외식","paymentMethod":null,"spentAt":"' + today + '","emoji":"🍖"}',
  ...
  try {
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: promptFor(mode, today, expenseCats, incomeCats) },
    ...
    return json({ ok: true, tx: { type, amount: Math.round(amount), category, memo, paymentMethod, spentAt, emoji } });
  } catch (e) {
