...
function promptFor(domain: 'beauty' | 'household' | 'recipe' | 'reading'): string {
  if (domain === 'reading') {
    return [
      '이것은 OCR 전사(transcription) 작업이다. 이미지는 책 페이지 사진이다.',
      '사진에 인쇄된 글자를 한 글자도 바꾸지 말고 그대로 옮겨라. 이것이 가장 중요한 규칙이다.',
      '- 의미가 어색하거나 비문처럼 보여도 절대 교정/완성/의역/요약하지 마라. 원문 그대로 베낀다.',
      '- 네가 아는 비슷한 표현으로 바꾸지 마라. 보이는 단어·어미·조사·문장부호·띄어쓰기를 있는 그대로 유지한다.',
      '- 확실히 읽히지 않는 글자는 추측해서 지어내지 말고, 보이는 부분까지만 옮긴다.',
      '',
      '그 위에서 본문을 문장 단위로 분리해 배열로 반환하라:',
      '- 한 문장은 마침표(.), 물음표(?), 느낌표(!) 또는 문맥상 완결되는 지점에서 끊는다.',
      '- OCR 줄바꿈만 제거한다 — 한 문장이 여러 줄에 걸쳐 있으면 이어 붙인다(줄바꿈 외에는 아무것도 바꾸지 않는다).',
      '- 따옴표(" ") 안의 대화/인용은 하나의 문장으로 유지한다.',
      '- 머리글, 바닥글, 페이지 번호는 본문에서 제외한다. 페이지 번호가 보이면 별도 필드로 추출한다.',
      '',
  ...
  try {
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        // reading 은 "원문 그대로 베끼는" 전사 작업 — temperature 0 으로 모델의 의역/교정(창의성)을 차단해 충실도↑.
        //   (나머지 도메인은 기존 기본값 1 유지 — 동작 변화 없음)
        temperature: domain === 'reading' ? 0 : 1,
