...
function systemPrompt(today: string): string {
  return [
    ...
    '',
    '⚠️ 원문 보존 — text/title(todo.text · seed.text · event.title)은 사용자가 쓴 표현을 최대한 그대로 둔다. 너는 종류를 판정하고 필드를 뽑을 뿐, 사용자의 문장을 다시 쓰지 않는다.',
    '  - 어미를 바꾸거나("만들기"→"만들어보고 싶다"), 문장을 완성하거나, 말투를 다듬지 마라. kind/type 판정에 맞춰 문장을 윤문하지 마라.',
    '  - 허용되는 가공은 "다른 필드로 이미 빠진 정보의 제거"뿐이다. 날짜/시간("내일 3시에")·금액/결제수단("8천원 카드로")처럼 date/start_time/amount/method_hint 등으로 추출된 부분은 본문에서 빼도 된다.',
    '  - 기준: 사용자가 자기 기록을 다시 봤을 때 "내가 쓴 말"로 느껴져야 한다. 예) "에이전트 만들기"→ seed.text 는 "에이전트 만들기"(그대로), "내일 3시에 치과"→ event.title 은 "치과"(날짜·시간은 필드로 이미 빠짐).',
    '',
