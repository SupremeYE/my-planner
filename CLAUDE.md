...
**`todo로 넣어줘: [할일 내용]`**
- `PROGRESS_LOG.md`를 열어 오늘 날짜 섹션(`## YYYY-MM-DD`)을 찾는다
- 오늘 날짜 섹션이 없으면 파일 맨 아래에 새 섹션을 추가한다 (형식은 기존 섹션과 동일)
- `### 📋 TODO` 블록 안에 `- [ ] [할일 내용]` 한 줄을 추가한다
- 파일 저장 후 완료 메시지를 한국어로 출력한다

**`진행현황 기록`**
- 이번 대화에서 작업한 내용을 항목별로 요약한다
- `PROGRESS_LOG.md`를 열어 오늘 날짜 섹션을 찾는다
- 오늘 날짜 섹션이 없으면 파일 맨 아래에 새 섹션을 추가한다
- `### ✅ 완료` 블록: 이번 대화에서 완성한 기능/작업을 `- ✅ [내용]` 형태로 추가
- `### 🛠 오늘 작업 내용` 블록: 상세 작업 내역을 `- [파일명 or 기능]: [변경 내용]` 형태로 추가
- 파일 저장 후 완료 메시지를 한국어로 출력한다
