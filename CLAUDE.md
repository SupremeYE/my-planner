...
### `진행현황 기록`
오늘 세션에서 작업한 내용을 요약해서
PROGRESS_LOG.md 오늘 날짜 섹션에 기록한다.
- 완료된 TODO → ✅ 완료 섹션으로 이동 (`- [x]`로 변경)
- 새로 추가/수정/삭제한 기능 → 🛠 오늘 작업 내용에 기록
- 다음 세션이 이어서 작업할 수 있도록 **"🔜 다음 세션 인수인계"** 섹션을 작성한다:
  - 현재 미완료 TODO 목록
  - 코드는 완성됐으나 외부 작업(예: Supabase SQL 실행)이 남은 항목
  - 다음에 이어서 작업할 기능/버그

**PROJECT_SPEC.md도 함께 업데이트한다:**
- 버그 수정된 항목 → `🔴 버그` 섹션에서 제거
- Supabase 연동 완료된 항목 → `❌ 메모리` → `✅` 로 변경
- 새로 추가된 Supabase 테이블 → `2-1. 연동된 테이블 목록` 및 `2-2. 컬럼 상세`에 추가
- 데이터 흐름도(섹션 3) 업데이트

기록 후 GitHub 커밋한다. 커밋 메시지: "docs: YYYY-MM-DD 진행현황 기록"
