...
| `/todos` | `TodosView` | 전체 할일 날짜별 그룹 (미지정 탭 제거됨) |
| `/inbox` | → `/todos` 리다이렉트 | 옛 빠른 수집함(`InboxView`)은 **제거됨** — 현재 `/inbox`는 `/todos`로 리다이렉트만 한다(`routes.tsx`). `InboxView`·`src/lib/inbox.ts`는 코드에 존재하지 않음 |
...
| `/beauty-care` | `BeautyCareView` | 뷰티 케어 — **모바일 구현(S5)**, PC는 플레이스홀더(S7). **셀프케어 게이지**(`SelfCareGauge`: `selfCareScore` 원형 게이지, 높음 💖/중간 🌿/낮음 🥀 색분기 + 이번주 N회·7칸 스파크 별도 레이어) · **스페셜케어**(`SpecialCareSection`/`SpecialCareSheet`: 2열 카드·`careStatus` 배지·'오늘 했어요'→`markCareDone`·주기 편집/삭제) · **화장품 보유함**(`BeautyShelf`: 가로 미리보기 임박순 + 전체보기 오버레이 3열·카테고리 필터, 새 라우트 없이 내부 토글, 사용기한 잔여=개봉일+사용기한(PAO) 계산) · 상세(`BeautyProductSheet`: 수정·재구매'또 샀어요'·다씀보관/복원·삭제) · 추가 FAB(`BeautyAddSheet`). `useBeauty` 훅(`beauty_products`·`beauty_special_care` Realtime). 사진 AI 등록은 S6 |
| `/housekeeping` | `HousekeepingView` | 살림 — **모바일 구현(S4)**, PC는 플레이스홀더(S7). **넛지 배너**(우선순위 소진>교체지남>저재고>오래된청소) · **생필품 재고**(`HouseholdStockSheet`+StockCard: 수량 바·± 스테퍼·곧 떨어짐/다 썼어요 배지·다시채움) · **소모품 교체주기**(`ConsumableCycleCard`: 도넛 링·교체함·주기설정) · **청소 히트맵**(`CleaningHeatmap`: 2열 먼지 단계 ✨≤3일/💨4~9일/🕸️≥10일·미실시) · 추가 FAB(`HousekeepingAddSheet`). `useHousekeeping` 훅(`household_items`·`consumable_cycles`·`cleaning_zones` Realtime). 사진 AI 등록은 S6 |
| `/money` | `MoneyView` | 하온 머니 — 독립 모듈 `src/features/money/`(18파일). 상단 **4탭 [가계부/자산/투자/계획]**, `useMediaQuery`로 모바일(`MoneyMobile`)/PC(`MoneyDesktop`) 조건부 렌더. **가계부**: 채팅형 자연어 입력(`ChatInputBar`→`money-parse` Edge Function)·요약(수입/지출/잔여)·급여일 기준 예산바(`getMoneyPeriod`)·지출 캘린더(무지출 ₩사선)·카테고리별 지출(대/소분류)·주/월 회고 배너. **자산**(`AssetPanel`): 통장/현금 CRUD. **투자**(`InvestPanel`): 투자 계좌(평가액/원금/수량, 시세 수동). **계획**(`PlanPanel`): 저축/투자 목표·대출 관리·월초 계획(`MoneyPlanSheet`: 수입→고정비차감→가용→통장분배→생활비한도). 관리 시트: 카테고리·고정비(외화 `fx-rate` Edge Function 환산/자동정산)·카드. `useMoney` 훅 = **11테이블 로드 + Realtime 11구독 + 전 CRUD + `parseAndAdd`**. 전역 FAB 숨김(채팅바와 겹침 방지) |
...
#### `/todos` — 할일 페이지
- "전체 할일": 날짜별 그룹 (오늘/내일/요일 레이블), 완료 항목 접기/펼치기. 탭 없음(단일 화면)
- 기존 "미지정 할일" 탭은 제거됨(중복 제거). 옛 `/inbox`(`InboxView`)도 **제거되어** 현재 `/inbox`는 `/todos`로 리다이렉트된다
...
| `money_categories` | 머니 — 카테고리 (parent_id self-FK 대/소분류, type expense/income, 지출11+수입4 프리셋 + 소분류) | `sort_order` ASC | ✅ (moneyDb.categories, /money) |
| `money_transactions` | 머니 — 거래 (category_id·fixed_cost_id FK, source chat/sms/manual/fixed, 외화 original_amount/currency/fx_rate) | `spent_at` DESC | ✅ (moneyDb.transactions, /money) |
| `money_accounts` | 머니 — 통장/현금/투자 (type deposit/cash/investment, 투자 invest_kind/principal/quantity, include_in_total/is_default) | `sort_order` ASC | ✅ (moneyDb.accounts, /money) |
| `money_cards` | 머니 — 카드 (credit/check, billing_day, unpaid_amount, linked_account_id FK) | `sort_order` ASC | ✅ (moneyDb.cards, /money) |
| `money_fixed_costs` | 머니 — 고정비 (외화 original_amount/currency/fx_rate·결제주기 cycle·category_id FK·is_variable) | `billing_day` ASC | ✅ (moneyDb.fixedCosts, /money) |
| `money_loans` | 머니 — 대출 (principal/balance/interest_rate·회차 total/paid_installments) | `created_at` DESC | ✅ (moneyDb.loans, /money) |
| `money_goals` | 머니 — 저축/투자 목표 (target/current_amount·deadline·linked_account_id FK) | `sort_order` ASC | ✅ (moneyDb.goals, /money) |
| `money_plans` | 머니 — 월초 계획 (기간별 1행, 예상수입/고정비/가용/저축·투자·생활비·생활비한도) | `period_start` DESC | ✅ (moneyDb.plans, /money) |
| `money_plan_allocations` | 머니 — 계획 분배 항목=통장 쪼개기 (plan_id·account_id FK·is_living) | `sort_order` ASC | ✅ (moneyDb.allocations, /money) |
| `money_reviews` | 머니 — 주/월 회고 (type·period·week_index·total_spent·next_adjustment jsonb) | `period_start` DESC | ✅ (moneyDb.reviews, /money) |
| `money_settings` | 머니 — 설정 (user_id PK, period_type payday/calendar·payday·monthly_budget·fx_alert_threshold) | — (싱글톤) | ✅ (moneyDb.settings, /money) |
...
| 화장품 보유함 (BeautyProduct) | ✅ | ✅ | ✅ | ✅ | ✅ 연동 (beauty_products, Realtime · 재구매/다씀보관 액션 · 사진 업로드는 S6) |
| 스페셜케어 (BeautySpecialCare) | ✅ | ✅ | ✅ | ✅ | ✅ 연동 (beauty_special_care, Realtime · markDone 주기 관리) |
| 생필품 재고 (HouseholdItem) | ✅ | ✅ | ✅ | ✅ | ✅ 연동 (household_items, Realtime · 수량 스테퍼/다시채움 · 사진은 S6) |
| 소모품 교체주기 (ConsumableCycle) | ✅ | ✅ | ✅ | ✅ | ✅ 연동 (consumable_cycles, Realtime · 교체/주기설정) |
| 청소구역 (CleaningZone) | ✅ | ✅ | ✅ | ✅ | ✅ 연동 (cleaning_zones, Realtime · 청소완료 히트맵) |
...
- **할일 페이지(`/todos`)** — 전체 할일 날짜별 그룹(미지정 탭 제거), 공통 TodoModal 날짜 네비, 프로젝트 배지
...
│
├── FloatingAddFab (전역 빠른 캡처 FAB — QuickAddInput 팝오버/바텀시트, Layout·LayoutC 마운트)
│   └── QuickAddInput (통합 빠른 입력 — 자연어 파싱 칩 미리보기 + addTodo/addEvent, TodosView·DailyView·FAB 공용)
│
