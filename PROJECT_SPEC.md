...
| `/diary` | `DiaryView` | 통합 일기 — 탭 3개(오늘 일기/질문일기/이날의 기억). **오늘 일기**(diary_v3 재설계): PC 2단(좌 최근 일기 타임라인 / 우 작성·읽기) · 모바일(주간 스트립 + 작성·읽기 + 최근 접기). 작성=A 모드(줄 없음·점 질감·자동확장), 읽기=D 모드(줄 친 종이) + 수정 전환. **질문일기**: 오늘의 질문 답변·질문 탐색 시트·지난 질문일기. **이날의 기억**: 1~5년 전 오늘 기록(5년 일기) |
