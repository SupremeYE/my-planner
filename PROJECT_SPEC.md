...
| `/diary` | `DiaryView` | 통합 일기 — 탭 3개(오늘 일기/질문일기/이날의 기억). **Stage 1**: 오늘 일기(자유일기) 작성·자동저장·최근 일기 리스트 구현, 질문일기·이날의 기억은 placeholder(Stage 2·3) |
...
| `exercises` | 운동 종목 마스터/카탈로그 (user_id null=전체 공용, name_ko null=미채택, free-exercise-db 873종목 import) | `name_ko`, `name_en` | ⬜ (Stage 0 DB only — UI 미연동) |
| `workout_logs` | 운동 세션 (한 날/한 종목, exercise_id FK) | `performed_on` DESC | ⬜ (Stage 0 DB only) |
| `workout_sets` | 운동 세트 (근력 weight/reps · 유산소 duration_min/distance_km, log_id ON DELETE CASCADE) | `set_no` ASC | ⬜ (Stage 0 DB only) |
| `routine_days` | 요일별 루틴 헤더 (day_of_week 1~7, unique(user_id,day_of_week)) | `day_of_week` ASC | ⬜ (Stage 0 DB only) |
| `routine_exercises` | 요일 루틴 종목 (routine_day_id ON DELETE CASCADE, exercise_id) | `sort_order` ASC | ⬜ (Stage 0 DB only) |
