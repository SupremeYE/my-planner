...
| `/todos` | `TodosView` | 전체 할일 날짜별 그룹 (미지정은 `/inbox` 전담 — 미지정 탭 제거됨) |
...
- 미지정(date===null)은 `/inbox` 가 전담 — 기존 "미지정 할일" 탭은 제거됨(중복 제거)
...
- **할일 페이지(`/todos`)** — 전체 할일 날짜별 그룹(미지정은 `/inbox` 전담, 미지정 탭 제거), 공통 TodoModal 날짜 네비, 프로젝트 배지
...
│   └── AllTodosTab (날짜별 그룹, 완료 접기/펼치기 — 미지정은 /inbox 전담)
│
├── InboxView (빠른 수집함 /inbox)
│   ├── QuickAddInput (통합 빠른 입력 — 자연어 파싱 칩 미리보기 + addTodo/addEvent, 전역 FAB 공용)
│   └── 항목 카드 (체크 동그라미 + 메타 칩 + triage 오늘/내일/날짜/완료/삭제)
│
