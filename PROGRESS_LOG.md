...
## 2026-08-22 — ⏱ 할일 완료 '실행 시각' 마커 (타임라인 DO)

### 🛠 구현 (2026-08-16 "do_* 자동 30분 제거"의 짝 — '얼마나'는 안 짓되 '언제'는 남긴다)

자동 30분 부여를 제거하면서 완료 체크가 `do_*` 를 전혀 안 남기게 됐다. "몇 분 걸렸다"(소요)는 지어내지 않되(§통계·비교), "몇 시에 했다"(시각)는 사실은 남기고 싶어 **길이 0 완료 시각 마커**를 도입한다.

- **조사(선확인)**: 기존 `do_start=do_end` 레코드 **6건 전부 `do_elapsed_sec=0`** — `renderBlock` 의 `isCheckOnlyDo`(=`doElapsedSec===0`)로 30px **블록**+✓원형으로 그려지던 상태(마커와 구분 안 됨). 집계는 이미 안전(`todoDoDurationSeconds`=0 → `aggregateActivity` 의 `sec<=0` 가드로 시간 축 제외, 건수 축은 태그 기준이라 잡힘).
- **완료 체크 시 마커 기록**: 실적 DO 가 없으면 `do_start=do_end=완료시각`, `doElapsedSec=0`(길이 0). 공용 헬퍼 `lib/todoDoDuration.completionMarkerPatch`(+`isCompletionMarker`)로 `DailyView`/`TodosView` 체크박스·상태 메뉴가 공유. 이미 소요가 기록된 DO 는 보존, `date` 없는(인박스) 할일은 마커 없음, 완료 해제 시 마커(길이 0)만 정리(실제 소요는 보존).
- **시각 마커 표현 신설**: `timeline/Timeline.renderDoMarker` — 블록이 아니라 **점(✓)+할일명+얇은 선 꼬리**, 점 중심이 실행 시각선. 드래그 불가(순간), 탭=편집. check-only DO 는 조기 반환으로 마커 렌더, 기존 30px 블록 분기 제거. 일간·주간(`renderBlock` 공유) 모두 적용.
- **예전 30분 버그 재발 아님**: 그건 소요를 30분으로 **지어내** 시간 축을 오염시켰고, 이번 마커는 **길이 0**이라 시간 축에 절대 안 잡히고 "얼마나 자주"의 **건수 축에만** 잡힌다(마커가 늘어도 시간 축 불변).
- **문서/하네스**: `DESIGN.md` §5 "타임라인 DO — 완료 시각 마커"(블록 vs 마커 대비 표) 등록. 렌더 하네스 시드에 블록 vs 마커 대비 데이터 추가 + `daily` 라우트에 **모바일 '타임블록' 탭 서브 컷**(원래 안 찍던 공백) 추가.

### ✅ 검증

- `vite build` 통과 · `lint:fonts`/`lint:colors`(라벤더 432 불변) 통과 · **`bun test` 101 pass**(신규 `todoDoDuration.test.ts` 7 + `timeAggregation` 마커 케이스 포함 — 마커 늘어도 시간 축 불변·건수만 증가 잠금).
- `render:check`(daily/todos) FAIL 0 — **PC 일간**·**모바일 타임블록** 양쪽에서 마커 2건(아침 스트레칭 08:15·이메일 회신 10:40)이 점+선으로, 실적 블록(매뉴얼 양식 공유 14:00~)과 구분되어 렌더. 헤더 **"실제 시간 45분"**(블록만 반영, 마커 0초 미포함) 확인.
...
머니 모듈이 문서엔 2026-06-27 "Stage 1~3 / 다음 예정"으로만 남아 **문서만 보면 미완성으로 오인**됐다(하네스는 커밋 `880107e`로 이미 등록됨). 코드·DB 실측 기준으로 문서를 현행화한다.

- **머니 실상태(코드·DB 실측)**: `src/features/money/` **18파일 ~4,920 LOC** · DB **11테이블**(초기 8 + 마이그레이션 7개로 `money_plans`·`money_plan_allocations`·`money_reviews` 및 소분류(`money_categories.parent_id`)·외화·투자 컬럼 확장) · Realtime **11구독**(`useMoney.ts`) · 4탭(가계부/자산/투자/계획) **CRUD 전부 실구현**(자산·투자·목표·대출·월초계획·주/월 회고·카테고리 대소분류·고정비 외화정산·카드) · Edge Function **`money-parse`(v3 ACTIVE)** + **`fx-rate`(v1 ACTIVE, 외화 런타임 환산)**. 실 데이터: 거래 4건 전부 `source='chat'`(파서 경로 실사용), categories 69(대분류11+소분류54+수입4).
- **렌더 하네스**: 머니는 이미 커밋 `880107e`("test: 렌더 하네스에 /money 라우트 추가")로 등록돼 있음 — feature-local `features/money/db` → `mock-money-db.ts` 리다이렉트 + `/money` 4탭(+'이전 기간' 기간연동 회귀검증). 이번엔 코드 변경 없이 **문서만 현행화**.
- **문서 갱신**: 본 PROGRESS_LOG(이 항목) + `PROJECT_SPEC.md`(라우트표 `/money` 추가·DB 레지스트리 머니 **11테이블**·상단 changelog) + `CLAUDE.md`(하네스 섹션에 "커버 경계" 한 줄 — 화면 렌더만 커버, 데이터 계층·서버 정렬 `.order` 미커버. 대상·시드·동작의 머니 서술은 880107e에 기존재).
...
- 머니 조사 시 로컬 `render:check --route=money` 로 4탭(PC/모바일 8샷) 시드 렌더 FAIL 0 재현 확인(main의 `mock-money-db` 하네스). 문서 변경뿐이라 빌드/린트 영향 없음.
- **미검증(명시)**: `money-parse`·`fx-rate` **라이브 호출은 sandbox egress 차단으로 미실행**(두 함수 모두 ACTIVE, 브라우저 사용자 세션에서 확인 예정).
...
do_* 자동 30분 부여 버그 제거 후 시간 축의 소스가 타이머·수동 입력뿐이라 시간 리포트/리뷰 시간 스트립이 거의 비어 있었다. 시각이 명확한 활동(회의·점심)은 이미 일정으로 등록되므로, 일정에 태그만 붙으면 그대로 집계된다. 그러나 일정 모달에 태그 UI 가 없어 붙일 방법이 없었다(태그 달린 이벤트 0건). 이 작업으로 그 경로를 연다.

- **집계 소스 선확인(⚠️)**: `timeAggregation.aggregateActivity` 가 **이미** 이벤트 `startTime~endTime` 을 track_time 태그로 집계(주석에도 "일정 모달 태그" 대비 명시). 시간 리포트(`useTimeReport`)와 주간·월간 리뷰 시간 스트립(`review/TrackTimeStrip`)이 **같은 함수**를 호출 → 별도 세션의 소스 통일이 반영돼 있어 **태그만 붙이면 자동 집계**되고 두 화면이 같은 숫자를 보인다. (스키마 변경 불필요 — `events.tags` 컬럼 기존 존재.)
- **`TagSelector.tsx` (신규)**: TodoModal 에 인라인이던 태그 UI(칩 토글 + "+ 새 태그" 생성 + 팔레트 + 편집/삭제)를 **그대로 추출**. 선택 상태만 `value`/`onChange` 로 부모가 소유, 태그 CRUD·팔레트(localStorage)는 내부. "새로 만들지 말 것" → 공용 컴포넌트 추출로 해석(두 모달이 동일 컴포넌트).
- **TodoModal**: 인라인 태그 블록·상태·헬퍼·팔레트 로직·미사용 import 제거 → `<TagSelector/>` 사용. **순수 이동**이라 UX 무회귀.
- **EventModal**: `selectedTags` state 추가 + **프로젝트 다음(색상 앞)** 에 `<TagSelector/>` 배치(할일 모달의 project→tags 인접과 정합) + payload `tags` 를 `selectedTags` 로 왕복.
- **렌더 하네스(상주)**: 일정 편집 모달 열기(`editEvent` 디스패치)·`/time-report` 라우트 추가, 시드 이벤트 2건에 track_time 태그(업무·공부) 부여.

### ✅ 검증
- `vite build`·`lint:fonts`·`lint:colors`(라벤더 466 불변) 통과, 단위테스트(timeAggregation 6·todoSnooze 6·todoPeriod 10) 통과.
- `render:check`(PC/모바일): 일정 모달에 태그 섹션 노출·업무 태그 선택 왕복 확인, "+ 새 태그" 존재. **⭐ 시각+track_time 태그 일정**(팀 스탠드업 30분·디자인 리뷰 60분) → **시간 리포트 총 1시간 30분** = **주간 리뷰 시간 스트립 1시간 30분(공부 1시간·업무 30분)** — 두 화면 동일 숫자(공용 aggregateActivity).
- 범위 밖(DateField 교체·반복 칩·색상 팔레트화·모바일 오토포커스·라일락 청소) 미변경.

---

## 2026-08-16 — 📊 태그 기반 활동 분석(시간 통일+건수+인사이트) + 🐛 완료체크 do_* 자동부여 제거 + 🧹 데이터 정리

### 🛠 구현

- **태그 활동 분석 — 시간 축 통일 + 건수 축 + 문장 인사이트** (main `d720f28`):
  - **집계 소스 통일**: 「시간 리포트」와 리뷰 「시간 스트립(TrackTimeStrip)」이 서로 다른 소스(리포트=할일 do_* / 스트립=이벤트)를 봐서 두 화면이 다른 답을 내던 문제. 공용 함수 `lib/timeAggregation.ts`(`aggregateActivity`)로 통일 — 소스=할일 `do_*`(+블록 dual-read)+이벤트 시각, `plan_*` 제외. `useTimeReport`가 이 엔진에 위임, 스트립도 같은 함수 호출 → 구조적으로 동일 숫자. 이벤트 시각 0건이라 기존 리포트 숫자 회귀 없음.
  - **건수(件) 축**: 시간="얼마나 오래"(실행 시각 기록분만), 건수="얼마나 자주"(태그만 붙으면 모두). 스트립에 두 축 병기(형태·단위 다르게: 스택 바 시간/분 vs 태그별 막대 N건).
  - **문장형 인사이트**(공용 훅 `useActivityInsights`, 최대 3줄, 공백 알림 우선): 공백 기준 상수화 `GAP_ALERT_DAYS`(주 14일≈2주/월 21일≈3주), 저이력 태그 제외 `GAP_MIN_HISTORY=3`(예: 뷰티케어 1건 → 노이즈 제외), 현재기간 기록<`INSIGHT_MIN_RECORDS(2)`면 빈 배열. 한글 받침 기반 조사(은/는·이/가) 자동. 리뷰·시간 리포트 양쪽 동일 문장.
  - 플래그 결정: `tags.watch` 컬럼 추가 없이 "이력 있는데 최근 조용해진 태그만 알림" 스마트 규칙(사용자 승인).
- **🐛 완료 체크만으로 do_* 자동 부여 제거** (main `d720f28`): 즉시 끝나는 일("일반쓰레기봉투 사기"·"문의하기")도 완료 체크만 하면 현재시각+30분(또는 PLAN 복사)이 do_start/do_end에 박혀 시간 집계가 조작값이 되던 버그.
  - `DailyView.handleTodoCheckboxAction` +30분/PLAN복사 분기 삭제, `lib/todoToggle.buildTodoToggleUpdate`는 상태만 토글(do_* 보존, 미완료 시 wipe도 제거). 시간 기록은 명시적 행위(DO 드래그·타이머 완주·수동 입력)로만. "완료했는데 0분"은 건수 축이 해결.
  - Philosophy B("완료 시 실적 없으면 0분 집계 회피") 재검토 결론: 0분을 피하려 30분 거짓말을 만드는 게 더 나쁨 → 규칙 폐기.

### 🧹 데이터 정리 (DB UPDATE, 코드 아님)

자동부여 추정 do_* **18건**을 `do_start=do_end=do_elapsed_sec=NULL` 처리(status=done·태그·plan 보존 → 건수 축엔 그대로 잡힘). 타이머 실측 10건·0분 기록 6건·기타는 미접촉.

**롤백 스냅샷 (실행 전 원본값)** — 되돌리려면 각 id에 아래 do_start/do_end 복원:

| id | 제목 | do_start | do_end | 유형 |
|---|---|---|---|---|
| gufgr5d | 저녁먹고 설거지하기 | 19:30 | 20:00 | auto_30min |
| ejmcgcv | 다이소가서 펀치사기 | 18:55 | 19:25 | auto_30min |
| vkjw5xr | 경제뉴스 2개 읽기 | 22:36 | 23:06 | auto_30min |
| hd6suve | 월세지원 신청 확인해보기 | 22:36 | 23:06 | auto_30min |
| a1ycg0q | RSA2026 이지바로 등록하기 | 15:00 | 15:30 | auto_30min |
| fc6w04v | 하온 프로젝트 수정하기 | 20:10 | 20:40 | auto_30min |
| eqokllq | 뉴스 대시보드 프로젝트 기능추가개선 작업하기 | 00:52 | 01:22 | auto_30min |
| pkn2kwr | 선우랑 저녁먹기(야근식대) | 21:34 | 22:04 | auto_30min |
| w19b5yj | 배민B마트 배달온거 집에 넣어두기 | 18:57 | 19:27 | auto_30min |
| kcf8uat | KPI 면담 자료 준비하기 | 14:02 | 14:32 | auto_30min |
| r1nh3ng | 성은씨 IP R&D 지출품의서 작성 가이드 안내 메일 보내기 | 12:58 | 13:28 | auto_30min |
| b9d6tzf | 모델 허브 리서치 다시 정리 후 jira에 작성 | 13:49 | 14:19 | auto_30min |
| nb1nh1r | 일반쓰레기봉투 사기 | 18:20 | 18:50 | auto_30min |
| vxab9pw | 프라이버시 연구과제 api key 전달(김홍비 대리) | 09:58 | 10:28 | auto_30min |
| 200ju11 | 국내 출장 보고서 결재 올리기 | 10:28 | 10:58 | auto_30min |
| 2s8dd0z | 대전 교통 연구비카드로 결제 | 22:31 | 23:01 | auto_30min |
| udz5vkx | 일일오전회의 | 09:30 | 09:40 | plan_copy (plan 09:30~09:40 보존) |
| 2aq1z8l | 🍴점심시간(비가 왕창옴) | 11:30 | 12:30 | plan_copy (plan 11:30~12:30 보존) |

정리 후: 태그 do_* 기록 19~20→11건, 8월 태그 시간 120분(전부 가짜 30분)→0분(→8월 시간축은 정직하게 비고 건수 축이 대신 채움), 전체 실측 899분/11건만 잔존. 잔여 검증=30분 잔여 0·DO==PLAN 잔여 0·타이머 10 보존·0분 6 미접촉·done/태그 보존.

### ✅ 검증
- `npm run build`(vite 6)·`lint:fonts`·`lint:colors`(466 불변) 통과. 테스트 88건(신규 `timeAggregation.test.ts` 6건 포함) 통과.
- 렌더 스크린샷(PC/모바일)로 두 축+인사이트 확인(시간 리포트↔리뷰 동일 숫자, 자기관리 7/17 공백 문장, 뷰티케어 노이즈 제외).
- 자체 판단: ① 미사용 파생함수(week/monthFocusReport) 정리 ② `todo_time_blocks`(0건, 타이머 미사용 죽은 경로)·`ProfileView` 집중시간(게이미피케이션)은 조사만·미변경 ③ 데이터 정리는 코드(자동생성 차단)를 main에 먼저 반영한 뒤 실행(정리 도중 재발 방지).
...
⑩ slot(아침/저녁/기타) 도입 이후 통계가 "현재 체중·7일/30일 대비·진행률"에서 같은 날 여러 slot 을 섞던 문제를 근본 수정. a160f56(결정적 정렬)은 결정성만 확보했고 비교가능성은 미해결(과거일에 아침이 없으면 저녁으로 폴백 → 아침↔저녁 갭이 대비값에 그대로 오염)이라, "기준 slot" 개념으로 재설계.

- **Stage 1(규칙 등록)**: DESIGN.md **§7.4 몸무게 통계 카드 — 기준 slot 라벨** 신설(기준=아침 고정, 최근 30일 아침 0건 시 저녁 전환 / 탐색 단위 = "기준 slot 기록이 있는 날", 같은 날 다른 slot 바꿔치기 금지, 기타 제외 / 라벨 필수). CLAUDE.md **"통계·비교 원칙(폴백≠비교)"** 절 추가.
- **Stage 2(로직 + 테스트)**: `dayRep` 폴백 → `referenceSlot`(아침 고정, 30일 아침 0건→저녁) + `refRecords`(기준 slot만) 방식 교체. `changeFrom` = 기준일 이전 "기준 slot 기록이 있는 날"과 비교(없으면 건너뛰고 더 과거, 비교 쌍 없으면 null). 현재 체중·진행률도 동일 기준값. 로직 8케이스 재검증(무회귀·다중slot 아침고정·과거 저녁 스킵→더 과거 아침·아침 쌍 없으면 null·저녁 전환·기타 제외).
- **Stage 3(UI 라벨)**: `statCard`에 caption 추가. 현재 체중 = `값 (M.d slot)`(예 "57.2 kg / 7.14 아침"), 대비 = `값 (기준 slot 기준)` / 없으면 `— + 사유`(예 "아침 기록 2회 이상 필요"). 기준을 숨기지 않음.

### ✅ 검증
- `npm run build`(vite 6) 각 Stage 통과, 폰트가드 0건. 통계 로직 8케이스 단위검증 통과.
- 실 DB(7.16 저녁55.4 / 7.14 아침57.2 / 7.14 저녁56.2)로 렌더 산출: 현재 체중 "57.2 kg (7.14 아침)"(최신 55.4 저녁 아님), 7일/30일 대비 "— (아침 기록 2회 이상 필요)", 최저·최고(월) 55.4/57.2. 스펙 §7.4 일치.
- 스코프 Theme H·토큰만(라벨 색=§7.2 증감 토큰), 비-H 영향 0. 하루 1건 데이터는 무회귀.

---

## 2026-06-29 — 📝 리뷰 & 기록 페이지 개선 — Stage 6 (행복 캡처 + 패턴뷰 + 폰트 마무리)

### 🛠 구현 (행복 캡처 2경로 + 돌아보기 행복 패턴뷰 — 리뷰&기록 재설계 메인 플로우 완료)

`happy_moments` 테이블·액션(`addHappyMoment`) 재사용으로 행복 캡처 진입점 2곳 + 돌아보기 ✨행복 패턴 미니뷰 추가. DB 스키마 변경 0, 외부 API 0, 색상 토큰만.

- **공용 캡처 모달(`HappyCaptureModal`)**: 한 줄 입력 + 음성(`useVoiceInput`) + 저장. 저장 시 `happy_moments` insert(`content` / `date`=오늘 `getLogicalToday()` / **`happened_at`=현재 시각 ISO(now)** / `created_at` 자동). 본문 폰트 NanumSquareRound. PC 중앙 / 모바일 바텀시트(`items-end lg:items-center`), 코랄(`t.danger`) 저장 버튼.
- **진입점 ① 모바일 FAB(6-1)**: 전역 `FloatingAddFab` 롱프레스 스피드다이얼에 **"✨ 행복한 순간"** 추가(모바일 전용). 탭 → 공용 모달 → 저장 후 토스트 "행복한 순간이 기록됐어요"(로컬 state 토스트, 앱 기존 패턴).
- **진입점 ② 돌아보기 버튼(6-2)**: ✨행복 필터 선택 시 패턴뷰 상단에 코랄 "＋ 지금 행복한 순간"(PC 주 입력 경로, PC/모바일 공통). 동일 공용 모달 → 저장 즉시 store 낙관적 갱신 + realtime으로 스트림/패턴뷰 반영.
- **행복 패턴 미니뷰(6-3)**: ✨행복 필터 **선택 시에만** 결과 상단 노출(전체/타 필터에선 숨김). 제목 "✨ 주로 이럴 때 행복을 느껴요" + **시간대별 막대**(아침05–11/오후11–17/저녁17–22/밤22–05, `happened_at` 시(時) 매핑, **NULL 백필 건 시간대 집계 제외**) + **요약 한 줄**(행복 N번 · 최다 시간대 · 최다 요일 date getDay). 집계는 현재 적용된 연도/검색 필터(`filtered`)와 연동. 시각 없는 건 있으면 "※ 시간대 막대는 시각 기록 N건만 반영" 주석.
- **행복 카드 시각(6-4, 확인)**: 카드 `a h:mm` 표시(Stage 5), NULL 백필 건은 날짜만.
- **폰트(6-5, 확인)**: `--font-nanum-round: 'NanumSquareRound', var(--font-pretendard)` 정적 `@import`(webfontworld CDN, `fonts.css`) 로드 확인. 리뷰 작성 내용 `BODY_FONT` 적용, 타 페이지 Gaegu 미변경, 런타임 주입 없음.

### ✅ 검증
- `npm run build`(vite 6) 통과. 모바일 FAB→입력→`happened_at`=now 저장→돌아보기 즉시 반영. PC 돌아보기 버튼 캡처(공용 컴포넌트). 음성 입력 + 저장 토스트. ✨행복 필터에서만 패턴뷰. 시간대 막대 NULL 제외, 요약(횟수·최다 시간대·최다 요일). 카드 시각 표시(NULL은 날짜만). 작성 폰트 NanumSquareRound, 타 페이지 영향 0. DB 스키마 변경 0, 신규 색 하드코딩 0(코랄=`t.danger`).

---

## 2026-06-29 — 🌸 감정 기록 페이지 "기록" 탭 PC 최적화 — Stage 0~3

### 🛠 구현 (`/mood` "기록" 탭만 — 캘린더/통계 탭, 4단계 모달 내부 로직 무변경)

감정 기록 "기록" 탭의 진입점·지난 기록 표현·본문 폰트를 PC 최적화 관점에서 재구성. 데이터 모델/쿼리/정렬 변경 0, 색상은 기존 토큰만, PC 분기는 `lg:`만 사용.

- **Stage 0(조사)**: 컴포넌트 `MoodView.tsx`(단일 파일) 구조·진입점 2버튼(우상단·빈상태)이 동일 4단계 `RecordSheet`를 여는 점·지난 기록 데이터 소스(`mood_records` 단일 쿼리, `created_at` 최신순, 배지=동일 행 단일 진실)·폰트 토큰 목록(Noto Sans KR **의미 토큰 부재** 확인)·Figma Make 파일 아님·PC/모바일 분기 현황 진단 후 보고.
- **Stage 1(추가 버튼 통합 → 퀵 입력 바)**: 우상단 코너 "+ 기록" 버튼 제거. "오늘 기록 (N건)" 헤더 아래에 **퀵 입력 바** 신설 — 전체 너비·카드 배경(`t.card`)·골드 점선 보더(`1.5px dashed t.accent`)·우측 코랄 원형 + 버튼(`t.danger`), 바 전체 클릭 시 기존 4단계 모달(`openNew`). 빈 상태 중앙 "+ 기록하기" CTA 제거 → 퀵 입력 바 아래 안내 문구만, 바는 기록 유무와 무관하게 항상 같은 자리. 중복 오픈 코드 `openNew()` 공용화(전역 FAB도 재사용).
- **Stage 2(지난 기록 미리보기 카드 + PC 2열)**: 신규 `PastRecordCard`(카드별 `expanded` 로컬 상태) — 헤더(이모지·날짜·시간 + 펼침/접기 chevron) + 감정/컨디션 배지 줄 + 본문 `line-clamp-2` 미리보기, 펼침 시 전체 본문·에너지·수정/삭제. 레이아웃 모바일 1열 / PC `lg:grid-cols-2`(`gap-3`·`items-start`). 최신순·`slice(0,10)`·fetch 무변경(표현 계층만).
- **Stage 3(본문 폰트 + 마무리)**: `fonts.css`에 의미 토큰 **`--font-noto-sans: 'Noto Sans KR', var(--font-gowun)`** 신설(일기 손글씨 Gaegu와 분리). `MoodView` 루트에 적용 → 본문/미리보기/배지/모달 텍스트가 Noto Sans KR(제목·통계 숫자는 명시된 `--font-gmarket` 유지). 모바일(퀵 입력 바 터치 영역·1열·배지 줄바꿈)·PC(2열 그리드·기존 레이아웃) 점검.

### ✅ 검증
- `npm run build`(vite 6) 통과(Stage 1·2·3 각각). 색·폰트 하드코딩 0(gold=`t.accent`/coral=`t.danger`/배지=기존 `EMOTION_CATEGORY_COLORS`, 폰트=신규 의미 토큰). PC 분기 `lg:`만, 캘린더/통계 탭·4단계 모달 내부·데이터 쿼리/정렬 무변경.

### ⚠️ 범위 밖(후속 후보)
- 지난 기록 페이징/더보기(현 `slice(0,10)` 유지). 캘린더/통계 탭 폰트 통일은 루트 적용으로 자동 반영되나 별도 점검 대상 아님.

---

## 2026-06-29 — 📝 리뷰 & 기록 페이지 개선 — Stage 5-보강 (돌아보기 PC 레이아웃 수정)

### 🛠 구현 (오버레이 PC 레이아웃 + 헤더 버튼 — 스타일만, 로직 무변경)

돌아보기 오버레이가 PC에서 `column-count:2`로 화면을 반만 갈라 카드가 과도하게 넓고 본문이 한 줄로 늘어지던 문제 수정(B안). 검색/필터/점프/데이터 소스 로직은 그대로.

- **헤더 검색 버튼(5B-1)**: PC(`lg`)는 `🔍 돌아보기` 텍스트 알약(card 배경 + border 테두리, 높이 40, 패딩 16) / 모바일은 기존 아이콘 정사각(38) 유지. `useMediaQuery` 분기.
- **PC 3열(5B-2)**: 결과 스트림 컬럼 수를 3단 분기 — PC(`min-width:1024px`) **3열** / 태블릿(`min-width:768px`) 2열 / 모바일 1열. `columnGap` 18px, 월 라벨 `columnSpan:all` 풀폭, 카드 `breakInside:avoid`. 본문 미리보기는 3줄 클램프 유지(긴 텍스트 한 줄 늘어짐 해소). 월 라벨 sticky는 단일 열(모바일)에서만 적용.
- **검색·필터 정렬(5B-3)**: 검색창/필터/콘텐츠 모두 동일 좌우 패딩(`px-4 lg:px-6`) 유지로 일관 정렬.

### ✅ 검증
- `npm run build`(vite 6) 통과. 헤더 버튼 PC 알약/모바일 아이콘, 결과 PC 3열·태블릿 2열·모바일 1열 자연 분기, 본문 3줄 클램프, 월 라벨 풀폭·`breakInside` 정상, 패딩 일관. Stage 5 로직(검색/필터/점프/소스) 무변경, 신규 색 하드코딩 0(토큰만).

---

## 2026-06-29 — 📝 리뷰 & 기록 페이지 개선 — Stage 5 (돌아보기 / 검색 — 헤더 🔍 아카이브)

### 🛠 구현 (헤더 🔍 진입 풀스크린 "돌아보기" 오버레이)

리뷰&기록 헤더의 🔍 버튼에 핸들러 연결 → 통합 기록 아카이브 오버레이(`ArchiveOverlay`). 4종 소스를 한 스트림으로 합쳐 날짜 역순·월별 그룹, 실시간 검색(하이라이트)·타입/연도 필터, 카드 탭 시 원본 위치로 점프. `useMediaQuery` 분기, 색상 토큰만, DB 변경 0(기존 store 데이터 읽기).

- **진입/오버레이 골격(5-1)**: 헤더 🔍(기존 placeholder) 활성화 → `archiveOpen` 풀스크린 오버레이(`fixed inset-0 z-50`, `t.bg`). 상단 ‹ 돌아가기 + "돌아보기" 타이틀. 닫기 시 직전 탭(일간/주간/월간) 그대로 복귀(탭 state 보존).
- **통합 데이터 소스(5-2)**: store의 `reviewRecords`(감사 `gratitude` / KPT / **과거 `daily_*` 데일리리뷰 읽기전용 승계**) + `happyMoments`(행복, **백필분 포함**) + `weeklyReviews`(주간) + `monthlyReviews`(월간, `achievement` 과거값 포함) → `ArchiveItem` 스트림. `review_records.happiness` 는 `happy_moments` 로 분리·백필됐으므로 중복 방지로 미포함(행복은 happy_moments 단일 출처).
- **검색(5-3)**: 검색창 비면 전체를 월별 그룹으로 표시, 입력 시 220ms 디바운스 실시간 필터 + 매칭 키워드 `<mark>` 하이라이트(토큰 `${t.accent}33`). 검색 대상 = 각 레코드 본문 텍스트 전체(감사 줄·KPT·daily·행복·주/월간 필드 결합).
- **타입 필터 + 연도(5-4)**: 칩 전체/🙏감사/✨행복/🔄KPT/📅주간/🗓월간 단일선택, 키워드와 **AND 결합**. 연도 드롭다운(데이터에 존재하는 연도)으로 범위 제한. (데일리(daily_*)는 전용 칩 없이 "전체"에서 노출 — 📔 배지)
- **결과 카드(5-5)**: 타입 배지(토큰 색: 감사=`success`/행복=`danger`/KPT=`accent`/주간=`info`/월간=`text`/데일리=`textMuted`, `${color}1f` 틴트) · 날짜(행복은 **시각**까지, `a h:mm`) · 본문 미리보기(3줄 클램프, 하이라이트). 탭 → `onJump`으로 일간(그 날짜)/주간(그 주 시작일)/월간(그 달)으로 전환 + 해당 탭 열기(`jump` nonce 주입, 동일값 재점프도 트리거). 빈 결과 "검색 결과가 없어요".
- **레이아웃(5-6)**: 모바일 1열 스트림(월 라벨 sticky) / PC 2열 매거진(`columnCount:2`, 월 라벨 `columnSpan:all` 풀폭, 카드 `breakInside:avoid`). 토큰만, 신규 색 하드코딩 0.
- **점프 배선**: `ReviewsView`에 `jump:{date,nonce}` state. `DayTab`/`WeekTab`/`MonthTab` 에 `jump` prop + `nonce` 변경 시 내부 날짜(state) 동기화 effect 추가(일간=날짜, 주간=`parseISO`, 월간=`startOfMonth`).

### ✅ 검증
- `npm run build`(vite 6) 통과. 통합 소스 4종 표시(기록 목록의 daily_*·happiness 백필분 승계). 검색 비었을 때 월별 그룹 / 입력 시 실시간 필터+하이라이트. 타입×키워드×연도 AND. 카드 → 원본 위치 점프(일간/주간/월간). 행복 시각 표시, 빈 결과 안내. 모바일 1열 / PC 2열 매거진. 배지 토큰 색, 하드코딩 0. DB 스키마 변경 0. 일간/주간/월간 탭 본문 무변경(점프 prop만 추가).
...
주간 통계를 집중시간 중심으로 재구성. 평균 기분 카드 제거, 작은 통계 2종 + 풀폭 집중시간 분석 블록(큰 숫자·요일별·태그별·증감) 신설. 집중/증감/요일별/태그별 **전부 시간 리포트 엔진(`aggregateRange`) 재사용** — 신규 집계 0.

- **평균 기분 카드 제거(3B-1)**: 주간 통계의 "평균 기분" 카드 삭제(기분은 주 단위 평균 의미 약함 → 향후 행복 패턴뷰). 관련 `mood_records` 주간 평균 조회/realtime/상태 코드 전부 제거(`weekMoods`/`loadMoods`/`moodAvg` 등). `ENERGY_EMOJI` 상수도 미사용이라 정리. (컨디션 배지의 일간 mood 읽기는 그대로 유지)
- **작은 통계 2종(3B-2)**: **할일 완료율**(done/전체 % + 막대 + `done/전체` 서브) / **습관 달성일**(목업 표기 `n/7` — 그 주 7일 중 습관 하나라도 체크한 날 수 + 막대). 2열 그리드(`StatCard`: 값+단위 small + 라벨(서브 인라인) + 얇은 막대). 목업 `.stat` 형태에 맞춤.
- **집중시간 분석 블록(3B-3, 풀폭)** — `useTimeReport.ts`에 `weekFocusReport(todos, tags, weekStart, weekStartsOn, todayStr)` 신규 export(전부 `aggregateRange` 재사용, `useTimeReport`가 'now' 고정이라 임의 주를 못 꺼내는 문제 해소):
  - **(a) 큰 숫자 + 증감**: 그 주 합계 시간(소수 h, 38px 큰 숫자 + small "h"). 직전 주 동일기간 비교(엔진과 동일한 `subWeeks` + `aggregateRange`)로 총합 증감 ▲/▼ 알약(테두리, 증가 `t.success` / 감소 `t.danger` / 0이면 "—"). 게이지 막대 없음. 서브 "지난주 N h 대비 · 하루 평균 N h"(÷7).
  - **(b) 요일별 막대**: 그 주 월~일 7일 각 `aggregateRange` 일별 합계로 막대 높이(상단 라운드, max 26px) + 막대 위 소수시간 라벨, 오늘 `t.danger` 강조, 0인 요일 빈 막대.
  - **(c) 태그별 가로 바**: `byCategory`(태그별 합산·많은 순) 상위 5개 + 6개 이상은 "기타" 합산. 바 색 = `tags.color`(track_time 태그, 하드코딩 0), 기타는 `t.textMuted` 토큰. 시간은 소수 h 우측 정렬.
  - **(d) 링크**: 우상단 "시간 리포트 자세히 →" → `/time-report`(SelfCareView) 네비게이트.
- **레이아웃(3B-4)**: 작은통계 2종 → 집중블록. 집중블록 내부 (b)(c)는 모바일 세로 스택(사이 1px 구분선) / PC `lg` 요일|태그 2열 grid `1.1fr 1fr`(`useMediaQuery`). 빈 상태: 그 주 집중 0이면 "이번 주 기록된 집중시간이 없어요" + 링크. 토큰만.
- **목업 정합(`focus-block-mockup.html`)**: ministats `.stat`(값+단위+라벨·서브+막대), `.focus` 큰 숫자/알약 증감/요일 막대(상단 라운드)/태그 가로바/PC `.splitviz` 2열·모바일 구분선까지 시각 정렬. 시간 표기는 목업과 동일한 **소수 시간("12.4", "5.2h")**.

### ✅ 검증
- `npm run build`(vite 6) 통과. 집중/증감/요일별/태그별 신규 집계 0(`aggregateRange` 단일 소스, 현재 주 기준 `useTimeReport('week')`와 동일 엔진·범위 → 숫자 일치). 신규 색 하드코딩 0(태그 색은 DB `tags.color`, 오늘/감소 강조는 `t.danger` 토큰; KPT Problem 라벨의 `#D4735A`는 기존 레거시 유지). 평균기분 관련 댕글링 참조 0(`fmtFocus`/`habitChecks` 등 정리). 회고/과거/네비게이터 등 Stage 3 결과물 무변경.
...
**뷰티 케어 · 살림 두 신규 메뉴 — 단계별 구현(Stage 1~5)**
공통 3엔진 공유: ① 수량 기반 재고 임계(곧 떨어져요/소진) · ② 주기 넛지(마지막 N일 + cycle days) · ③ 사진→AI 자동등록(S6로 보류).

- **Stage 1 — DB 마이그레이션**: 뷰티(`beauty_products`, `beauty_special_care`) · 살림(`household_items`, `consumable_cycles`, `cleaning_zones`) 테이블 + RLS(단일 UID) + `supabase_realtime` publication 등록 + 사진 버킷(`beauty-photos`, `household-photos`).
- **Stage 2 — 데이터 레이어 전용(화면 변화 0)**: `store.tsx` 타입 5종(BeautyProduct/BeautySpecialCare/HouseholdItem/ConsumableCycle/CleaningZone) · `db.ts` 5모듈(fetchAll/upsert/delete + repurchase/markDone/updateQuantity/refill/replace/markCleaned 등 도메인 액션, snake_case 매핑) · `careUtils.ts` 신설(순수 함수 daysSince/careStatus/isLowStock/isDepleted/selfCareScore) · 훅 2개(useBeauty/useHousekeeping, Realtime 구독 + 파생값 메모이즈).
...
- **Stage 4 (전역 빠른 캡처 FAB)** — `FloatingAddFab` 재작성: `+` 버튼 → **PC 팝오버 / 모바일 바텀시트**에 `QuickAddInput`(defaultDate 기본 null=Inbox). ESC·바깥클릭·dim 탭·저장 시 닫힘, `+`는 열릴 때 45° 회전. `onAddTodo`/`onAddEvent` 는 선택적 "상세" 단축으로 노출(있을 때만, 하위 호환 — 날짜 맥락 상세 모달 유지). `Layout`(A/B/D) 모바일+PC main·`LayoutC`(C) main 에 전역 마운트, `/daily`·`/calendar` 는 자체 FAB 보유로 제외(중복 방지). DailyView/CalendarView 무변경 — 기존 FAB 가 자동으로 빠른 입력+상세 단축이 됨. `AddEntryMenu`(헤더 상세 드롭다운)와 공존. FAB 는 fixed/absolute 라 레이아웃 무영향.
- **Stage 5 (할일 "미지정" 탭 제거)** — 미지정(date===null)은 Inbox 가 전담하므로 `TodosView` 의 `UnassignedTab` + 탭 UI(탭 토글/카운트) 제거 → "전체 할일"(날짜 배정분) 단독 렌더. `TodoRow` 의 미지정 전용 날짜배정 로직(showDateAssign/onAssignDate/assign 패널/CalendarClock) 제거. `AllTodosTab`·모달·`AddEntryMenu`·데이터 모델 무변경. ※기존 미지정 탭이 함께 보여주던 `status==='backlog'` 할일은 TodosView 에서 빠짐(Inbox 도 backlog 제외, `BacklogView` 는 기존처럼 미라우팅 — backlog 노출 위치는 추후 결정).
