...
## 2026-07-17 — ⭐ BUGFIX-1 Part 2-fix: 승인 결정(별=그날 계획) 이탈 수정

### 🐛 문제 (Part 2가 옛 제안대로 구현 → 승인 결정 이탈)
정책: **별(`is_top3`)은 "그날 계획"의 속성이지 "할일"의 속성이 아니다.** Part 2는 이 정책 대신 폐기된 옛 제안(자리 있으면 유지 / 무피드백 클램프)을 따라 4곳이 어긋나 있었다.

### 🛠 수정
- **수정 1·2 — 미루기(별 미노출 → 부작용): 항상 해제 + 알림.** 미루기 3경로(`DailyView` SnoozeModal·빠른미루기, `CalendarView` 미루기)에서 날짜가 바뀌면 자리 유무·캡 검사와 무관하게 `isTop3: false`로 강제하고, 별이 있었다면 토스트로 알린다. (舊 "자리 있으면 유지"·"무피드백 클램프" 제거.)
- **수정 3 — 모달 날짜 변경(별 노출 → 의도): 자리 있으면 유지, 꽉 차면 해제 + 알림.** `TodoModal`은 초기 마운트를 `prevDateRef`로 건너뛰고, 실제 날짜 변경 시에만 판정 → 옮긴 날에 자리 있으면 별 유지, 꽉 찼으면 해제하고 별 옆에 사유(`옮긴 날은 이미 핵심 3개예요. 별은 해제했어요`) 인라인 노출.
- **수정 4 — `TodosView` 무피드백 no-op 제거.** `toggleTop3`가 `Top3ToggleResult`(`{ ok, reason }`)를 반환하도록 변경(舊 `return prev`로 조용히 끝나던 것). 두 탭(할일/완료함) 모두 `at-cap` 반환 시 토스트로 사유 표시. (사전 비활성 미도입 근거: DESIGN.md에 근거 패턴 부재 → 프롬프트 지침대로 "토스트만".)

### 🔧 알림 구조 (택: **a + c 혼합**)
- **(a) 판정을 setter 밖으로** — 미루기·모달은 호출부(이벤트 핸들러)가 현재 스냅샷으로 판정·알림. 리듀서 순수성 유지, strict mode 이중 발화 무관(핸들러는 이중 실행 안 됨).
- **(c) 쓰기 함수가 결과 반환** — `toggleTop3` → `Top3ToggleResult`. 직접 토글 경로(`TodosView`)의 무피드백 no-op 해소.
- 공통 알림 수단: `useKeyHint` 훅(신설, `src/app/hooks/useKeyHint.tsx`) — 기존 DailyView 힌트 pill을 재사용 가능한 훅으로 승격(DailyView 자체도 이 훅으로 통일). CalendarView·TodosView 두 탭에 동일 pill 호스팅. **새 디자인 패턴 아님**(기존 pill 재사용).

### 📝 최종 문구 3개 (`store.TOP3_MSG`)
1. `핵심 할일은 하루 3개까지예요` (캡 도달 — 별 토글/직접 지정 거부)
2. `핵심 표시는 그날 계획에 남아요. 옮긴 날에 다시 지정해주세요` (미루기 해제)
3. `옮긴 날은 이미 핵심 3개예요. 별은 해제했어요` (모달 날짜 변경·꽉 참)

### 📋 `2/3` 카운트 표시 여부
**미도입 — 근거 없음.** DESIGN.md에 Top3 카운트 노출 패턴이 없어(§ 확인) 임의 추가하지 않음.
...
- `WeeklyView`(주간 드래그 리스케줄, :600) — 드래그로 날짜 이동. 미루기와 동일 성격이나 별도 알림 호스트 필요.
→ 권장: 위 셋을 "미루기(D2)"와 같은 규칙(항상 해제 + 알림)으로 통일하는 후속 태스크. `useKeyHint` 훅이 이미 재사용 가능하므로 각 뷰에 pill 호스팅만 추가하면 됨.

### ✅ 검증
- `npm run build`(vite 6) 통과, `npm run lint:fonts` 0건.
- 미루기 3경로: 자리 남은 날로 미뤄도 별 해제 + 알림. 모달: 자리 있으면 유지 / 없으면 해제 + 알림. `TodosView`: 캡 도달 시 사유 토스트.
- strict mode: 알림은 이벤트 핸들러에서만 발화 → 이중 표시 없음. 모달 마운트는 `prevDateRef`로 초기 오발화 방지.
