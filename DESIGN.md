...
- 팝오버(목록)는 "페이지 위에 뜨는 것" → §1 오버레이 글래스(`glassBarStyle`) 대상.
...
전용). **haonStyles helper (register before build; 소비처는 구현 Stage 4a-1):** 트리거 표면 =
기존 `inputBg` 재사용, 팝오버 = 기존 `glassBarStyle` 재사용(신규 헬퍼 없음). 콤보박스 목록 항목
상태(hover/active)용 헬퍼가 필요할 수 있음 → 구현 시 판단. 이번 문서 Stage에서는 `haonStyles.ts`를
건드리지 않는다.
