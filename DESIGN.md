...
> **테마 H 전용 계약.** (A/B/C/D 는 대상 아님 — 위 규격은 H 에만 적용한다.)

카테고리 색(`--cat-*`)과 **별개의 축**이다. 카테고리 색은 **"무엇인가"**(할일/일정/습관/자기관리)를,
상태 색은 **"지금 어떤 상태인가"**(늦음 등)를 나타낸다. 두 축은 서로 대체하지 않는다.

**경계 (중요).** 임박·주의는 **기존 `warning` 토큰(`#F6C177`, §3 Semantic status)을 계속 사용**한다.
`--state-*` 는 현재 **"늦음(late / overdue)" 하나만** 정의한다. 새 상태를 추가하기 전에 기존 semantic
status(`warning` = 임박·주의 / `danger` = 위험·삭제·실패)로 표현 가능한지 **먼저 확인**한다. 표현이
안 될 때만 `--state-*` 네이밍을 따라 신설한다.

**토큰 정의 (신규 hex 최소화 — 팔레트 참조 + 파생).** 늦음 표면은 §3 팔레트에 이미 있는 **소프트 코랄**
(`t.accentLight` = `#F6BCBA`)을 **직접 참조**한다. 같은 색에 두 이름을 주지 않기 위해 새 hex 를 만들지
않고 `withAlpha` 파생으로 처리한다. 텍스트만 대비 확보용으로 신규 hex 1개를 둔다.

| 역할 (`--state-*`) | 값 | 비고 |
|---|---|---|
| `--state-late-bar` | `t.accentLight` (소프트 코랄 `#F6BCBA`) | 기존 팔레트 **직접 참조** — 신규 hex 아님 |
| `--state-late-bg` | `withAlpha(t.accentLight, 0.42)` | 파생 (칩 배경) |
| `--state-late-border` | `withAlpha(t.accentLight, 0.55)` | 파생 (카드 테두리) |
| `--state-late-text` | `#A8433E` | **유일한 신규 hex** — 대비 확보용이라 파생 부적합 |

- **`--state-late-text` = `#A8433E` 대비 근거 (실측).** 칩 배경(`--state-late-bg` = 소프트 코랄 42% over
  흰 카드 = 실효 `rgb(251,227,226)`) 위에서 **4.86:1** — WCAG AA(4.5:1) 충족. (당초 후보 `#B4514C` 는 동일
  배경에서 **4.07:1** 로 AA 미달이라 채택하지 않고 `#A8433E` 로 조정했다.)
- **단색 틴트만.** 상태 색에 액센트 그라데이션(`#FF9A8B`→`#FF6F91`)을 쓰지 않는다.
- **액센트와 역할이 다르다.** 소프트 코랄과 시각적으로 인접하지만 — **액센트 = 사용자가 누를 것**
  (FAB, 주 액션, 선택일), **상태 색 = 시스템이 알리는 것**(늦음). 상태 색을 인터랙티브 어포던스로 쓰지 않는다.
- **좌측 3px 액센트 바 판별 규칙 (중요).** 좌측 3px 세로 바는 §5 만다라트·List row 에서 이미 쓰이는
  시각 관용구다. 색으로 의미를 구분한다:
  > `--cat-*` 계열 = **분류**(무엇인가). `--state-*` 계열 = **상태**(지금 어떤가).
  > 한 요소에 두 종류를 **동시에 적용하지 않는다.**
- **늦음 카드 표현:** 좌측 3px 세로 바(`--state-late-bar`) + 카드 테두리(`--state-late-border`).
- **늦음 칩:** 배경 `--state-late-bg`, 텍스트 `--state-late-text` — 나머지 규격(pill·크기·weight 등)은
  §5 기존 **Chip / tag** 계약을 그대로 상속한다.
...
### 미루기 팝오버 (Reschedule Popover)

> **테마 H 전용 계약.** (A/B/C/D 는 대상 아님.)

할일 행에서 "언제로 미룰지"를 고르는 떠 있는 메뉴.

**표면.** §1 오버레이 글래스 규정에 따라 **`addPopoverStyle` 을 그대로 상속한다.** 자체 표면 수치
(alpha·blur·radius·shadow)를 **재기술하지 않는다** — 재기술하면 값이 두 곳에 살아 드리프트가 생긴다.
...
| padding | 6px |
| width | 214px 고정 |
| 섹션 헤더 | 10.5px / 600 / `t.textMuted` / padding 6px 10px 5px |
| 항목 | padding 9px 10px / radius 11px |
| 항목 라벨 | 12.5px / 500 / 딥 인디고(`t.text`) |
| 항목 우측 힌트 | `fontNumeric`(Sora) 10.5px / `t.textMuted` — 날짜 등 |
| 아이콘 | 15px / stroke 1.8 / `t.textMuted` |
| 구분선 | 1px hairline(`t.border`) / margin 5px 8px |
| press 상태 | §5 Interaction states(위) 기존 계약 상속 |

등장 애니메이션:
```
transform-origin: top right
scale(0.94) translateY(-6px) → scale(1) translateY(0)
0.2s cubic-bezier(.32,.72,0,1)
```

**배치.** 트리거 버튼 우측 정렬, 기본은 버튼 하단 6px. 하단 공간이 부족하면 버튼 위로 뒤집는다.

**인터랙션 규격 (이 팝오버에 한정하지 않고 앱 공통으로 채택):**
- 짧게 탭 = 기본 동작 즉시 실행
- 길게 누르기 **420ms** = 팝오버 열림 + 햅틱 8ms `(잠정 — 실기기 검증 후 확정)`
- 데스크톱(`lg:` 이상)은 롱프레스 대신 버튼 우하단 chevron 어포던스를 노출한다.
...
다음 주 월요일
──────────
날짜 선택 →   ← 기존 DateField 팝오버 재사용, 신규 피커 금지
```
늦음 상태 항목에 한해 최상단에 `오늘 하기`를 추가하고 구분선으로 분리한다.

### 하위 작업 리스트 (Subtask List)

> **테마 H 전용 계약.** (A/B/C/D 는 대상 아님.)

부모 할일 행 카드 **내부**에 포함되는 종속 목록.

**표면 판별 기준 (List row 계약의 별도 케이스).** §5 List row 의 `"흰 카드 안에 중첩된 행 =
t.surfaceMuted"` 규칙을 하위 작업에 그대로 적용하지 않는다. 예외가 아니라 **별도 케이스**다:
> 중첩 행의 표면은 그 행이 **독립적으로 읽히는지**로 결정한다.
> **독립적으로 읽히는 데이터 행**(거래 내역 등) = `t.surfaceMuted`. 부모와 동등한 무게를 갖는다.
> **부모에 종속된 목록**(하위 작업 등) = **표면 없음.** 들여쓰기와 가이드라인으로만 계층을 표현한다.
> 종속 목록에 배경을 주면 부모와 시각적으로 동급이 되어 계층이 무너진다.

하위 작업은 **자체 카드 표면을 갖지 않는다**(카드 안의 카드 금지).

```
컨테이너 padding   0 12px 10px 40px
가이드라인         left 23px / width 1.5px / hairline 계열(`t.border`) / radius 1px
행 padding         7px 2px
자식 체크박스       15px / radius 5px / 테두리 1.6px
자식 텍스트         12.5px / 400 / 본문보다 한 단계 옅게(`t.textSub`)
완료 표현          §5 List row 기존 계약 상속
                   (배경 불변 + 체크 + 취소선 + `t.textMuted`)
```

펼침:
```
max-height 0 → 280px, 0.3s cubic-bezier(.32,.72,0,1)
chevron 13px / stroke 2.2 / rotate 0 → 90deg / 0.24s
```

추가 행(`+ 하위 작업 추가`): 15px 점선 테두리 박스(`t.border`) + 12.5px `t.textMuted` 텍스트.
**목록 최하단에 항상 노출**한다 — 개수 제한이 없음을 시각적으로 드러내기 위함.

**체크박스 축 구분 (중요).** §5 Selection mode(위)의 원형 체크박스와 형태가 겹친다. 다음을 명시한다:
> 원형 체크박스는 두 축에서 쓰인다.
> **완료 체크박스** = 부모 할일 행, 19px 원형, 카테고리 색 테두리.
> **선택 체크박스** = 선택 모드, hairline outline / `t.accent` 채움.
> 하위 작업의 15px **각진(radius 5px)** 체크박스는 **완료 축**이며, 형태로 부모와 계층을 구분한다.

**동작 규칙:**
- 깊이는 **1단계로 고정**한다. 하위의 하위를 만들지 않는다.
- 개수 제한은 없다.
- **부모 완료는 수동이다.** 자식을 전부 완료해도 부모 체크박스는 변하지 않으며, **어떤 안내·강조·상태
  변화도 표시하지 않는다.**
  - 근거: 하온 할일은 완료 타임스탬프가 곧 실제 소요시간 집계의 근거다(Philosophy B). 자동 완료 시
    마지막 자식을 체크한 시각이 부모의 완료 시각으로 잘못 기록된다.

### 진행률 칩 (Progress Chip)

> **테마 H 전용 계약.** (A/B/C/D 는 대상 아님.)

부모 행이 하위 작업을 가질 때 완료 정도를 참고로 보여주는 작은 칩.

```
폰트     fontNumeric(Sora) 10.5px / 500
색상     t.textMuted
배경     t.surfaceMuted   ← 실제 토큰 참조. hex 하드코딩 금지
padding  2.5px 7px
radius   7px
```

- §5 **"Surface fills — surfaceMuted vs lavenderTint"**(위) 규칙을 상속한다 — 배경은 의미 없는 면 구분이라
  `t.surfaceMuted`(중립 그레이)를 쓰고 `t.lavenderTint`(라일락)를 쓰지 않는다.
- **액센트 색과 카테고리 색을 쓰지 않는다.** 진행률은 참고 정보이며 행동을 압박하는 요소가 아니다.
- 부모 행 제목 우측에 배치한다.
