...
**진행 링.**
- **트랙**: 저채도 라일락 (`t.lavenderTint`), 링 두께 **13px**, 반지름 **104**(236px 정사각 캔버스 기준).
...
- 링 트랙의 라일락은 만다라트 진행 트랙 규정("라일락 진행 트랙 금지" → 중립 `surfaceMuted`)과 **다른
  맥락**이다: 만다라트 트랙은 곁다리 인디케이터(면 구분 → 중립)지만, 전체화면 포커스 링은 화면의 **단일
  히어로/강조 요소**라 §3 `lavenderTint` 의 "선택·활성·강조" 허용 분기에 해당한다. 코드에서는 해당 라인에
  `// lint-colors-ok: 전체화면 포커스 링 트랙(히어로 강조)` 를 단다.
...
**Scope:** 모바일 전용, Theme H, 기존 토큰만(신규 토큰 없음). 라벤더 소비는 링 트랙 1건(위 `lint-colors-ok`).
