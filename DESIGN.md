...
습관 하루는 **달성/미달성 이진값**이라 진하기(농담)가 표현할 정보가 없다 → **달성 셀은 습관마다
톤 하나씩만** 쓰고 채도 단계를 두지 않는다(습관색은 하나, 그 안에서 농담 단계 없음). (GitHub
잔디식 다단계 채도는 하루 다중 기록이 있을 때만 성립하며 여기선 해당 없음.) 세 상태만 정의하고,
**형태·크기로 구분**되게 하여 범례(legend)를 두지 않는다:

| 상태 | 표면 | 형태 |
|---|---|---|
| **달성** | **습관별 지정 색**(`habit.color`, 아래 「습관 색상 팔레트」) 채움, **테두리 없음** | 꽉 찬 셀 |
| **미달성** | 흰/저채도 빈 칸(`t.card`) + **헤어라인만**(`t.border`) | 빈 셀(면 유지) |
| **해당없음** (미래·비해당 요일) | 뮤트(`t.textMuted` / `t.surfaceMuted`) | **크기를 줄인 점** — 면을 채우지 않음 |

- **달성 셀 색 = 습관별 지정 색(`habit.color`).** 미달성·해당없음은 색과 무관하게 위 표대로 고정
  (습관색은 **달성 셀에만**). **미지정 습관의 기본색**은 팔레트 첫 색(스카이 `#A5CDE9`).
- **변경 이유(단일 톤 결정을 뒤집음).** 이전엔 "달성 = 코랄 단일 톤"이었다(트래커는 전부 습관이라
  카테고리 구분이 없고, 코랄은 §3의 "selected-day tint / emphasis" 축으로 정당). 그러나 ① 실행 탭
  체크원은 이미 습관별 색을 쓰고 있어 **같은 '달성' 표현이 탭마다 달라** 어긋났고, ② 습관이 많으면
  코랄 단일 톤 트래커에서 **행·카드 간 습관 식별이 색으로 되지 않았다**. 그래서 달성 표현을 습관별
  색으로 통일한다. **오늘 요일 강조·스트릭 배지는 계속 코랄(`t.accent`)** — 습관색(달성)과 강조색을
  분리해 역할이 겹치지 않게 한다.
- **습관 색상 팔레트(§3 보강).** **명도·채도 통일(HSL S≈60%·L≈78% 고정), 색상(hue)만 회전**한
  파스텔 세트로 습관색을 고른다. 통일된 L/S 덕에 어느 색이 달성이어도 **밝기·무게가 같아** 한 화면에
  섞여도 균질하다(특정 습관만 튀지 않음). 밝은 파스텔이라 채움 위 텍스트/체크는 `onFill`(휘도>160 →
  딥 텍스트)로 대비를 확보한다. lavender-mist 밴 4색(`#F4E7FB`/`#EFE3FA`/`#EDE4F7`/`#E8E0F6`)만 피하면
  hue는 자유(트래커 셀·실행 체크원 공용). 구 저장값은 `normalizeHabitColor` 매핑으로 소비 시점 리맵
  (store 불변·지연 마이그레이션).
...
- **위계 = 습관 이름이 행에서 가장 큰 활자.** 현재는 이름이 가장 작아 위계가 역전돼 있다(수정 대상).
  ...
  `fontNumeric`(Sora, §4).
- **7일 셀** = **22~24px 정사각**, 균등 간격, **요일 헤더는 상단 1회.** 오늘 요일만 코랄(`t.accent`)로
  강조한다. 행 높이는 셀에 맞춰 조여 습관 8개 이상이 한 화면에 들어오게 한다(밀도 우선).
...
- **미니 히트맵** = 정사각 셀 그리드, **셀 간격 < 셀 크기**(붙어 보이게), **흘림(flowing) 배치**. 달성
  셀 색은 위 「달성 셀 색 규칙」(코랄 단일 톤)을 그대로 쓴다. 범위:
  - **월간** — 1일부터 말일까지 **순서대로** 흐르게 채운다. **요일 오프셋(선행 빈칸)을 두지 않는다**
    — 달력 정렬(요일 패턴 확인)은 주간 탭 A안의 역할이고, 월간 카드 상단에 뜬 빈 칸은 깨진 것처럼
    보인다. 셀 **12~14px**, 카드 폭에 맞춰 한 행 **11~16칸**. 미래 날짜도 같은 그리드에 점으로
    포함해(같은 흐름) 행을 통째로 차지하지 않게 한다.
  - **연간** — 12개월 × 각 월 일수만큼 칸을 늘린다(월=행, 일=열의 흘림). 셀은 월간보다 작다(~6~8px).
- **탭 간 밀도 계조:** 주(22~24px) → 월(12~14px) → 연(~6~8px). 같은 표현이 탭마다 6배씩 차이 나지
  않도록 완만한 계조를 유지한다(연간 밀도가 기준).
...
`habit.color`(달성 셀, 습관 팔레트), `t.accent`(오늘 요일·스트릭 강조), `t.card`/`t.border`(미달성 빈
칸), `t.textMuted`/`t.surfaceMuted`(해당없음 점·주기 라벨), `fontNumeric`(달성 합계·날짜), `onFill`(습관색
채움 위 텍스트/체크 대비), `mixHex`(필요 시 뮤트 톤 파생). **신규 haonStyles 헬퍼·색 토큰 없음.**
