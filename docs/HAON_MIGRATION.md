...
3. **종료일 필드 — 반복 없을 때도 항상 노출.** `end_date`(멀티데이)와 `recurrence_end_date`(반복 종료)가
   별개 컬럼이나, UI 노출 정책이 반복 트랙과 얽혀 미결(반복 트랙 소관). *(종료일 영어 포맷 건은 §11.3 참조.)*
...
- **결론:** 순수 표시 포맷 버그가 아니라, (a) 종료일이 빈 상태 + (b) 헤드리스/en 로케일 아티팩트. 실제 한국
  ...
  무효 실증). 옵션: (A) 네이티브 유지(표준·일관), (B) 종료일에도 한국어 동반 라벨 추가(입력 플레이스홀더는
  그대로), (C) 커스텀 date 컴포넌트 도입(별도 결정·현 스코프의 "새 컴포넌트 금지"와 상충). → 사용자 확인 대기.
