...
- ~~스토어 `toggleTop3`(하드 3) vs DailyView `toggleKeyTodo`(소프트 토스트) 캡 정책 불일치.~~ → BUGFIX-1(2026-07-17)에서 하드 통일로 해소.
- Dashboard '중요' 카운트는 `starred` 필드 사용(≠ `isTop3`).
- **반복 할일 별(`is_top3`) 표시/카운트 불일치** (BUGFIX-1 Part 2-fix 기록, 미수정):
  `recurrenceExpansion.ts:89`(`...parent`)가 부모의 `isTop3`를 **모든 가상 인스턴스에 복사**한다. 따라서 매일 반복 할일에
  별을 달면 **모든 날짜에 별이 표시**되지만, 캡 카운트(`countTop3ForDateIn`)는 **부모 date 버킷 1회만** 센다.
  → 반복에서만 별이 "그날 계획"이 아니라 "할일" 속성처럼 동작 → **승인된 정책과 어긋남.**
  현재 DB 상 반복 top3 = **0건**이라 실사용 영향 없음(마이그레이션·수정 안 함).
  **해소안 제안(구현 보류):** ① 표시 정합 — 확장 시 부모 date(또는 예외 레코드)에서만 `isTop3` 유지, 그 외 인스턴스는
  `isTop3: false`로 마스킹(가장 단순, 정책 부합). 또는 ② 별을 인스턴스별 예외로 승격(각 날 독립 지정 가능) — DO 기록을
  날짜별 독립 처리한 방식과 대칭이나 데이터 모델 확장 필요. **①을 권장**(정책 "별=그날 계획"과 직접 부합, 데이터 무변경).
